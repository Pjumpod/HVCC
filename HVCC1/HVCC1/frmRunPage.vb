...
#Region "[[  Transection Zone  ]]"
    ...
    Private Sub ShowPrintManual()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t1.Model,t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Serial,t1.EachCount")
        ...
        With cmd
            ...
            dr = .ExecuteReader()
            If dr.Read() Then
                cmbCustomer1.Text = dr.GetString(1)
                cmbCustomer2.Text = dr.GetString(2)
                cmbPartNo.Text = dr.GetString(3)
                cmbDetail.Text = dr.GetString(4)
                cmbCode.Text = dr.GetString(5)
                cmbLeakTest.Text = dr.GetString(6)
                cmbQuantity.Text = dr.GetInt32(7)
                cmbLotSize.Text = dr.GetInt32(8)
                cmbSerial.Text = dr.GetString(9)
                cmbSerial2.Text = dr.GetString(9)
                cmbCounter.Text = dr.GetString(10)
                cmbCounter2.Text = dr.GetString(10)
            Else
    ...
    Private Sub ShowPrintQueue()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t1.Model,t2.AutoModel,t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Counter,t1.Serial,t2.AutoModel,t1.EachCount,t1.Print")
        sb.Append(" FROM (tbPrintLabel t1")
        sb.Append(" INNER JOIN tbPartNo t2")
        sb.Append(" ON t1.PartNo = t2.PartID)")
        sb.Append(" INNER JOIN tbCustomer t0")
...
#Region "[[[   Run Serial   ]]]"
    ...
    Private Sub UpdateManualCounter()
        ...
        With Conndb
            ...
            .Open()
        End With
        Dim sqlCounter As String = "SELECT Counter,EachCount FROM tbPrintLabel WHERE [Model]='" & cmbModel.Text & "'" ' AND [Order]='" & lbOrder.Text & "'"
        da = New OleDbDataAdapter(sqlCounter, Conndb)
        da.Fill(dsCount, "tbPrintLabel")
        For Each row As DataRow In dsCount.Tables("tbPrintLabel").Rows
            If dsCount.Tables("tbPrintLabel").Rows(0)("Counter").ToString <> "" Then
                Dim sqlupdteCount As String = "Update tbPrintLabel SET [Counter]='" & txtCounter1.Text & "',[EachCount]='" & cmbCounter.Text & "'"
                sqlupdteCount = sqlupdteCount & " WHERE [Model]='" & cmbModel.Text & "'" ' AND [Order]='" & lbOrder.Text & "'"
                Dim cmdUpdate As OleDbCommand = New OleDbCommand(sqlupdteCount, Conndb)
                cmdUpdate.ExecuteNonQuery()
            End If
        Next
...
#Region "[[[LOT Print]]]"
    ...
    Private Sub Label30X550Matrix()
        ...
        pText.AppendLine("^A0,40,35")  ' font h,w
        pText.AppendLine("^FD" & "" & txtCustomer1.Text & "^FS") ' Customer part NO.
        ...
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbLotPrinter.Text) 'print the string USB   ZDesigner ZM400 300 dpi (ZPL)     
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB HVCC1---Zebra ZM400 (300 dpi) - ZPL----
    End Sub
    ...
    Private Sub Label30X550MatrixManual()
        ...
        pText.AppendLine("^XZ") ' End Print
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL) (Copy 1)") 'print the string USB
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbLotPrinter.Text) 'print the string USB HVCC1
    End Sub
...
#Region "[[[Each Print]]]"
    ...
    Private Sub Label55X25Matrix()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(txtCustomer1.Text, 4))
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Right(txtCustomer1.Text, 5))

        pText.Remove(0, pText.Length)
        pText.AppendLine("^XA")   ' Start new document
        pText.AppendLine("^PW1000")   ' Print W=100

        pText.AppendLine("^FO30,30") ' X,Y position
        pText.AppendLine("^A0N,120,100")  ' font h,w
        pText.AppendLine("^FD" & "" & lbCode.Text & "^FS") ' Code

        pText.AppendLine("^FO150,120") ' X,Y position
        pText.AppendLine("^A0N,50,65")  ' font h,w
        pText.AppendLine("^FD" & "" & Cus11 & "^FS") ' Customer No.1 DB2N

        pText.AppendLine("^FO150,200") ' X,Y position
        pText.AppendLine("^A0N,50,65")  ' font h,w
        pText.AppendLine("^FD" & "" & Cus12 & "^FS") ' Customer No.1 61130

        'Bar Code-------------------------------------------------
        pText.AppendLine("^FO410,30") ' X,Y position
        pText.AppendLine("^BX,6,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & Cus11 & "-" & Cus12 & " " & txtPartNo1.Text & " " & tsToday.Text & " " & lbCounter.Text & " " & lbShift.Text & "^FS") 'Current Matrix barcode
        '-----------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC1 printer
    End Sub
    Private Sub Label55X25Matrix2()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(txtCustomer1.Text, 4)) 'EB3B
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Mid$(txtCustomer1.Text, 5, 6)) '18D283
        Dim Cus13 As String = Format(Microsoft.VisualBasic.Mid$(txtCustomer1.Text, 12, 2)) 'BC
        ...
        pText.AppendLine("^FD" & "" & lbCode.Text & "^FS") ' Code B

        pText.AppendLine("^FO150,120") ' X,Y position
        pText.AppendLine("^A0N,50,65")  ' font h,w
        pText.AppendLine("^FD" & "" & Cus11 & "^FS") ' Customer No.1 EB3B

        pText.AppendLine("^FO150,200") ' X,Y position
        pText.AppendLine("^A0N,50,65")  ' font h,w
        pText.AppendLine("^FD" & "" & Cus12 & "^FS") ' Customer No.1 18D283

        pText.AppendLine("^FO350,200") ' X,Y position
        pText.AppendLine("^A0N,50,65")  ' font h,w
        pText.AppendLine("^FD" & "" & Cus13 & "^FS") ' Customer No.1 BC

        'Bar Code-------------------------------------------------
        pText.AppendLine("^FO410,30") ' X,Y position
        pText.AppendLine("^BX,5,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & Cus11 & "-" & Cus12 & "-" & Cus13 & " " & txtPartNo1.Text & " " & tsToday.Text & " " & lbCounter.Text & " " & lbShift.Text & " " & txtCustomer2.Text & "^FS") 'Current Matrix barcode
        '-----------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
    ...
    Private Sub Label55X25QR()
        ...
        pText.AppendLine("^FDQA," & "" & Cus11 & "-" & Cus12 & " " & txtPartNo1.Text & " " & tsToday.Text & " " & lbCounter.Text & " " & lbShift.Text & "^FS") 'Current QR barcode
        '------------------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB     
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC1---Zebra ZM400 (300 dpi) - ZPL---
    End Sub
    Private Sub Label55X25QR2()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(txtCustomer1.Text, 4)) 'EB3B
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Mid$(txtCustomer1.Text, 5, 6)) '18D283
        Dim Cus13 As String = Format(Microsoft.VisualBasic.Mid$(txtCustomer1.Text, 12, 2)) 'BC
        ...
        pText.AppendLine("^BQ,2,5")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & Cus13 & "-" & Cus11 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & " " & lbgsdb.Text & "^FS") 'Spare QR barcode
        pText.AppendLine("^FDQA," & "" & Cus11 & "-" & Cus12 & "-" & Cus13 & " " & txtPartNo1.Text & " " & tsToday.Text & " " & lbCounter.Text & " " & lbShift.Text & " " & txtCustomer2.Text & "^FS") 'Current QR barcode
        '------------------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB     
    ...
    Private Sub Label55X25MatrixManual()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(cmbCustomer1.Text, 4))
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Right(cmbCustomer1.Text, 5))
        ...
        pText.AppendLine("^FO410,30") ' X,Y position
        pText.AppendLine("^BX,6,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & Cus11 & "-" & Cus12 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbCounter.Text & " " & lbShift.Text & "^FS") 'Current Matrix barcode
        '-----------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC1 printer
    End Sub
    Private Sub Label55X25MatrixManual2()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(cmbCustomer1.Text, 4)) 'EB3B
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Mid$(cmbCustomer1.Text, 5, 6)) '18D283
        Dim Cus13 As String = Format(Microsoft.VisualBasic.Mid$(cmbCustomer1.Text, 12, 2)) 'BC
        ...
        pText.AppendLine("^FO30,100") ' X,Y position
        pText.AppendLine("^A0N,150,130")  ' font h,w
        pText.AppendLine("^FD" & "" & cmbCode.Text & "^FS") ' Code B
        ...
        pText.AppendLine("^FO410,30") ' X,Y position
        pText.AppendLine("^BX,5,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & Cus11 & "-" & Cus12 & "-" & Cus13 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbCounter.Text & " " & lbShift.Text & " " & cmbCustomer2.Text & "^FS") 'Current Matrix barcode
        '-----------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
    ...
    Private Sub Label55X25QRManual()
        ...
        pText.AppendLine("^BQ,2,5")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & Cus13 & "-" & Cus11 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & " " & lbgsdb.Text & "^FS") 'Spare QR barcode
        pText.AppendLine("^FDQA," & "" & Cus11 & "-" & Cus12 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbCounter.Text & " " & lbShift.Text & "^FS") 'Current QR barcode
        '------------------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC1 printer
    End Sub
    Private Sub Label55X25QRManual2()
        Dim Cus11 As String = Format(Microsoft.VisualBasic.Left(cmbCustomer1.Text, 4)) 'EB3B
        Dim Cus12 As String = Format(Microsoft.VisualBasic.Mid$(cmbCustomer1.Text, 5, 6)) '18D283
        Dim Cus13 As String = Format(Microsoft.VisualBasic.Mid$(cmbCustomer1.Text, 12, 2)) 'BC
        ...
        pText.AppendLine("^BQ,2,5")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & Cus13 & "-" & Cus11 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & " " & lbgsdb.Text & "^FS") 'Spare QR barcode
        pText.AppendLine("^FDQA," & "" & Cus11 & "-" & Cus12 & "-" & Cus13 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbCounter.Text & " " & lbShift.Text & " " & cmbCustomer2.Text & "^FS") 'Current QR barcode
        '------------------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbEachPrinter.Text) 'print the string USB
...
#Region " [[[Processing of getting ShortType array from StringType array of multiline TextBox]]] "
    ...
    End Function
#End Region
#Region " [[[Processing of Read button for 32bit Integer]]] "
        ...
        If Val(txtModel.Text) <> 0 Then
            If Val(txtCounter1.Text) < Val(txtSetquantity1.Text) - 1 Then
                ...
                For Each li In lvPrint.Items
                    If li.SubItems(0).Text = txtModel.Text Then 'And li.SubItems(1).Text = txtOrder.Text Then
    ...
    Private Sub CountQuantityManual()
        If Val(txtModel.Text) <> 0 Then
            If Val(txtCounter1.Text) < Val(txtSetquantity1.Text) - 1 Then
                ...
                txtCounter1.Text = cnt1
                For Each li In lvPrint.Items
                    If li.SubItems(0).Text = txtModel.Text Then
                        li.SubItems(10).Text() = cnt1
                        li.subitems(11).text() = lbSerialNo1.Text
                        li.subitems(13).text() = lbCounter.Text
                        li.BackColor = Color.LightGreen
