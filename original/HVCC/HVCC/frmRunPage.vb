...
#Region "[[  Transection Zone  ]]"
    ...
    Private Sub ShowPrintManual()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t1.Model,t1.Order,t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Serial,t1.EachCount")
        ...
        With cmd
            ...
            If dr.Read() Then
                ...
                cmbCounter1.Text = dr.GetString(11)
    ...
    Private Sub ShowPrintQueue()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t1.Model,t1.Order,t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Counter,t1.Serial,t2.AutoModel,t1.EachCount")
        sb.Append(" FROM (tbPrintLabel t1")
        sb.Append(" INNER JOIN tbPartNo t2")
        sb.Append(" ON t1.PartNo = t2.PartID)")
        sb.Append(" INNER JOIN tbCustomer t0")
    ...
    Private Sub SaveDataLogAuto()
        ...
        sb.Append("'" & txtPartNo1.Text & "','" & txtCustomer1.Text & "','" & txtCustomer2.Text & "','" & Val(txtSetquantity1.Text) & "','" & lbCode.Text & "','" & txtLeakTest.Text & "','Auto')")
    ...
    Private Sub SaveDataLogManual()
        ...
        sb.Append("INSERT INTO tbDataLog([timestamp],[Shift],[Routing],[PartNo],[CustomerNo1],[CustomerNo2],[Counter],[Code],[LeakTest],[PrintMode])")
        sb.Append(" Values('" & dmy.ToString("yyyy-MM-dd HH:mm:ss") & "','" & lbShift.Text & "','" & lbRouting.Text & "',")
        sb.Append("'" & txtPartNo1.Text & "','" & txtCustomer1.Text & "','" & txtCustomer2.Text & "','" & Val(txtSetquantity1.Text) & "','" & lbCode.Text & "','" & txtLeakTest.Text & "','Manual')")
        Dim sqlPrint As String = sb.ToString()
        Dim cmdInsert As OleDbCommand = New OleDbCommand(sqlPrint, Conndb)
        cmdInsert.ExecuteNonQuery()
        Beep()
...
#Region "[[[LOT Print]]]"
    ...
    Private Sub Label30X550Matrix()
        ...
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & lbSerialNo1.Text & "|" & txtLotSize1.Text & "^FS") 'Current barcode
        ...
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL) (Copy 1)") 'print the string USB   ZDesigner ZM400 300 dpi (ZPL)     HVCC---Zebra ZM400 (300 dpi) - ZPL----
...
#Region "[[[Each Print]]]"
    ...
    Private Sub Label55X25Matrix()
        ...
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB
    End Sub
    Private Sub Label55X25QR()
        ...
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL)") 'print the string USB     HVCC---Zebra ZM400 (300 dpi) - ZPL---
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB
    End Sub
    ...
    Private Sub Label55X25MatrixManual()
        ...
        pText.AppendLine("^FO400,40") ' X,Y position
        pText.AppendLine("^BX,6,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & Cus21 & "-" & Cus11 & "-" & Cus12 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & " " & lbgsdb.Text & "^FS") 'Current Matrix barcode
    ...
    Private Sub Label55X25QRManual()
        ...
        pText.AppendLine("^BQ,2,5")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & Cus13 & "-" & Cus11 & " " & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & " " & lbgsdb.Text & "^FS") 'Spare QR barcode
        pText.AppendLine("^FDQA," & "" & cmbPartNo.Text & " " & tsToday.Text & " " & cmbSerial.Text & " " & lbShift.Text & "^FS") 'Current QR barcode
        '------------------------------------------------------------------
        'pText.AppendLine("^PQ2") ' Print Quantity 2 copy
        pText.AppendLine("^XZ") ' End Print
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL)") 'print the string USB
