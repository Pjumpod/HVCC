...
Public Class frmRunPage
    ...
    '#End Region
    Private Sub frmRunPage_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Timer2.Start()
        Opacity = 1
    End Sub
    Private Sub frmRunPage_Deactivate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Deactivate
        Timer2.Stop()
        Opacity = 0.5
    End Sub
...
#Region "[[  Transection Zone  ]]"
    ...
    Private Sub ShowPrintManual()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Serial,t1.EachCount,t2.PartID")
        ...
        sb.Append(" WHERE t2.PartID=" & cmbPartID.Text & "")
        ...
        With cmd
            ...
            dr = .ExecuteReader()
            If dr.Read() Then
                cmbCustomer1.Text = dr.GetString(0)
                cmbCustomer2.Text = dr.GetString(1)
                'cmbPartNo.Text = dr.GetString(2)
                cmbDetail.Text = dr.GetString(3)
                cmbCode.Text = dr.GetString(4)
                cmbLeakTest.Text = dr.GetString(5)
                cmbQuantity.Text = dr.GetInt32(6)
                cmbLotSize.Text = dr.GetInt32(7)
                cmbSerial.Text = dr.GetString(8)
                cmbSerial2.Text = dr.GetString(8)
                cmbCounter.Text = dr.GetString(9)
                cmbCounter2.Text = dr.GetString(9)
                'cmbPartID.Text = dr.GetString(10)
            Else
    ...
    Private Sub ShowDataLog()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT TOP 100 timestamp,Shift,Routing,PartNo,CustomerNo1,CustomerNo2,Code,Counter,LeakTest,PrintMode")
        ...
        With cmd
            ...
            If dr.HasRows Then
                dt = New DataTable()
                dt.Load(dr)
                dgvDatalogger.DataSource = dt
            Else
                dgvDatalogger.DataSource = Nothing
            End If
        End With
        dr.Close()
        Conndb.Close()
    ...
    Private Sub ShowPrintQueue()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t2.Code,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Counter,t1.Serial,t1.PartNo,t1.EachCount,t1.Print")
        sb.Append(" FROM (tbPrintLabel t1")
        sb.Append(" INNER JOIN tbPartNo t2")
        sb.Append(" ON t1.PartNo = t2.PartID)")
        sb.Append(" INNER JOIN tbCustomer t0")
        sb.Append(" ON t0.CusID = t1.CustomerNo")
        'sb.Append(" WHERE t2.PartNo='" & txtPartNo1.Text.Trim() & "'")
        sb.Append(" WHERE t2.PartNo LIKE '%" & txtPartNo1.Text.Trim() & "%'")
        Dim sqlOrder As String = sb.ToString()
        cmd = New OleDbCommand()
        With cmd
            .CommandType = CommandType.Text
            ...
            Do While datReader.Read()
                ...
                txtCustomer1.Text = objListItem.SubItems(0).Text
                txtCustomer2.Text = objListItem.SubItems(1).Text
                txtPartNo1.Text = objListItem.SubItems(2).Text
                txtPartDetail1.Text = objListItem.SubItems(3).Text
                lbCode.Text = objListItem.SubItems(4).Text
                txtLeakTest.Text = objListItem.SubItems(5).Text
                txtSetquantity1.Text = objListItem.SubItems(6).Text
                txtLotSize1.Text = objListItem.SubItems(7).Text
                txtCounter1.Text = objListItem.SubItems(8).Text
                lbSerialNo1.Text = objListItem.SubItems(9).Text
                txtPartID.Text = objListItem.SubItems(10).Text
                lbCounter.Text = objListItem.SubItems(11).Text
                lbPrintUse.Text = objListItem.SubItems(12).Text
                Dim i As Integer = 0
                While (i < lvPrint.Items.Count)
                    lvPrint.BeginUpdate()
                    RemoveListViewLine(i, lvPrint.Items(i).SubItems(2).Text, lvPrint.Items(i).SubItems(4).Text)
                    i = i + 1
                    lvPrint.EndUpdate()
                End While
            Loop
    ...
    Private Sub RemoveListViewLine(ByVal n As Integer, ByVal TextCrit As String, ByVal SubCrit As String)
        ...
        While (n < lvPrint.Items.Count)
            li = lvPrint.Items(n)
            If li.SubItems(2).Text = TextCrit And li.SubItems(4).Text = SubCrit Then
    ...
    Private Sub SaveDataLogAuto()
        ...
        sb.Append("'" & txtPartNo1.Text & "','" & txtCustomer1.Text & "','" & txtCustomer2.Text & "','" & lbSerialNo1.Text & "','" & lbCode.Text & "','" & txtLeakTest.Text & "','Auto')")
    ...
    Private Sub SaveDataLogManual()
        ...
        sb.Append("INSERT INTO tbDataLog([timestamp],[Shift],[Routing],[PartNo],[CustomerNo1],[CustomerNo2],[Counter],[Code],[LeakTest],[PrintMode])")
        sb.Append(" Values('" & dmy.ToString("yyyy-MM-dd HH:mm:ss") & "','" & lbShift.Text & "','" & lbRouting.Text & "',")
        sb.Append("'" & cmbPartNo.Text & "','" & cmbCustomer1.Text & "','" & cmbCustomer2.Text & "','" & cmbSerial.Text & "','" & cmbCode.Text & "','" & cmbLeakTest.Text & "','Manual')")
        Dim sqlPrint As String = sb.ToString()
        Dim cmdInsert As OleDbCommand = New OleDbCommand(sqlPrint, Conndb)
        cmdInsert.ExecuteNonQuery()
        Beep()
...
#Region "[[[   Run Serial   ]]]"
    Private Sub AutoSerialUp()
        ...
        End With
        ds.Clear()
        sqlRunNumber = "SELECT t1.Serial FROM (tbPrintLabel t1 INNER JOIN tbPartNo t2 ON t1.PartNo = t2.PartID) WHERE t2.PartNo='" & txtPartNo1.Text.Trim() & "'" '[Model]='" & lbModel.Text & "' AND [Order]='" & lbOrder.Text & "'"
        da = New OleDbDataAdapter(sqlRunNumber, Conndb)
        da.Fill(ds, "tbPrintLabel t1")
        For Each row As DataRow In ds.Tables("tbPrintLabel t1").Rows
            If ds.Tables("tbPrintLabel t1").Rows(0)("Serial").ToString = "" Then
                ds.Tables("tbPrintLabel t1").Rows(0)("Serial") = "0000"
                RunNumber = "0000"
            Else
                RunNumber = Format(Microsoft.VisualBasic.Right(ds.Tables("tbPrintLabel t1").Rows(0)("Serial").ToString, 4) + 1, "0000")
            End If
        Next
    ...
    Private Sub UpdateSerial()
        With Conndb
            ...
            .Open()
        End With
        Dim sqlSerial As String = "SELECT Serial FROM tbPrintLabel WHERE [PartNo]=" & Val(txtPartID.Text) & "" '[Model]='" & lbModel.Text & "' AND [Order]='" & lbOrder.Text & "'"
        ...
        For Each row As DataRow In ds.Tables("tbPrintLabel").Rows
            If ds.Tables("tbPrintLabel").Rows(0)("Serial").ToString <> "" Then
                ...
                Sql = Sql & " WHERE [PartNo]=" & Val(txtPartID.Text) & ""
    ...
    Private Sub AutoCounterUp()
        ...
        End With
        dsECount.Clear()
        sqlRunNumber = "SELECT t1.EachCount FROM (tbPrintLabel t1 INNER JOIN tbPartNo t2 ON t1.PartNo = t2.PartID) WHERE t2.PartNo='" & txtPartNo1.Text.Trim() & "'"
        da = New OleDbDataAdapter(sqlRunNumber, Conndb)
        da.Fill(dsECount, "tbPrintLabel t1")
        For Each row As DataRow In dsECount.Tables("tbPrintLabel t1").Rows
            If dsECount.Tables("tbPrintLabel t1").Rows(0)("EachCount").ToString = "" Then
                dsECount.Tables("tbPrintLabel t1").Rows(0)("EachCount") = "0000"
                RunCounter = "0000"
            Else
                RunCounter = Format(Microsoft.VisualBasic.Right(dsECount.Tables("tbPrintLabel t1").Rows(0)("EachCount").ToString, 4) + 1, "0000")
            End If
        Next
    ...
    Private Sub UpdateCounter()
        ...
        With Conndb
            ...
            .Open()
        End With
        Dim sqlCounter As String = "SELECT Counter,EachCount FROM tbPrintLabel WHERE [PartNo]=" & Val(txtPartID.Text) & ""
        ...
        For Each row As DataRow In dsCount.Tables("tbPrintLabel").Rows
            If dsCount.Tables("tbPrintLabel").Rows(0)("Counter").ToString <> "" Then
                ...
                sqlupdteCount = sqlupdteCount & " WHERE [PartNo]=" & Val(txtPartID.Text) & ""
    ...
    Private Sub EjectSerial()
        ...
        If lvPrint.SelectedItems.Count > 0 Then
            lv0 = lvPrint.SelectedItems(0).SubItems(10).Text
        ...
        With Conndb
            ...
            .Open()
        End With
        Dim sqlSerial As String = "SELECT Serial FROM tbPrintLabel WHERE [PartNo]=" & lv0.ToString & "" '[Model]='" & lv0.ToString & "' AND [Order]='" & lv1.ToString & "'"
        da = New OleDbDataAdapter(sqlSerial, Conndb)
        da.Fill(ds, "tbPrintLabel")
        For Each row As DataRow In ds.Tables("tbPrintLabel").Rows
            If ds.Tables("tbPrintLabel").Rows(0)("Serial").ToString <> "" Then
                Sql = "Update tbPrintLabel SET [Serial]='0000',[Counter]='0',[EachCount]='0000'"
                Sql = Sql & " WHERE [PartNo]=" & lv0.ToString & "" '[Model]='" & lv0.ToString & "' AND [Order]='" & lv1.ToString & "'"
                Dim cmdUpdate As OleDbCommand = New OleDbCommand(Sql, Conndb)
                cmdUpdate.ExecuteNonQuery()
                ClearDisplay()
            End If
    ...
    Private Sub RunItemNo()
        ...
        For row = 0 To dgvDatalogger.RowCount - 2
            dgvDatalogger.Rows(row).Cells(0).Value = row + 1
        Next
    End Sub
...
#Region "[[[LOT Print]]]"
    ...
    Private Sub Label30X550Matrix()
        ...
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbLotPrinter.Text) 'print the string USB AS
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL) (Copy 1)") 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB HVCC_Radiator---Zebra ZM400 (300 dpi) - ZPL----
    End Sub
    Private Sub Label30X550MatrixP()
        pText.Remove(0, pText.Length)
        pText.AppendLine("^XA")   ' Start new document
        pText.AppendLine("^PW1000")   ' Print W=50
        pText.AppendLine("^FWR") ' Rotate 90
        ...
        pText.AppendLine("^FO350,300") ' X,Y position
        pText.AppendLine("^A0,65,60")  ' font h,w
        pText.AppendLine("^FD" & "" & txtPartNo1.Text & "P" & "^FS") ' Part No.
        ...
        pText.AppendLine("^FO50,280") ' X,Y position
        pText.AppendLine("^A0,45,40")  ' font h,w
        pText.AppendLine("^FDLot No.: " & "" & lbShiftDate.Text & "|" & txtPartNo1.Text & "P" & "|" & lbRouting.Text & "|" & lbSerialNo1.Text & "^FS") ' Lot No.
        'Bar Code-------------------------------------------------
        pText.AppendLine("^FO50,80") ' X,Y position
        'pText.AppendLine("^BX,5,200")  ' Matrix Code
        'pText.AppendLine("^FD" & "" & txtCustomer1.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & lbSerialNo1.Text & "|" & txtSetquantity1.Text & "|Auto ^FS") 'Reserve new barcode
        pText.AppendLine("^BX,8,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & "|" & txtPartNo1.Text & "P" & "|" & lbRouting.Text & "|" & lbSerialNo1.Text & "|" & txtSetquantity1.Text & "^FS") 'Current barcode
        pText.AppendLine("^FO50,1300") ' X,Y position
        'pText.AppendLine("^BXB,4,200")  ' Matrix Code Rotate 90
        'pText.AppendLine("^FD" & "" & txtCustomer1.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & lbSerialNo1.Text & "|" & txtSetquantity1.Text & "|Auto ^FS") 'Reserve new barcode
        pText.AppendLine("^BXB,5,200")  ' Matrix Code
        ...
        pText.AppendLine("^FD" & "" & txtPartDetail1.Text & "^FS") ' Lot No.
        pText.AppendLine("^FO200,1400^A0N,20,20") ' X,Y position
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & " " & "" & txtPartNo1.Text & "P" & " " & "" & lbRouting.Text & " " & "" & lbSerialNo1.Text & "^FS") ' part No.
    ...
    Private Sub Label30X550QR()
        ...
        pText.AppendLine("^FDQA," & "" & lbShiftDate.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & lbSerialNo1.Text & "|" & txtLotSize1.Text & "^FS") 'Current barcode
        pText.AppendLine("^FO50,1300") ' X,Y position
        ...
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbLotPrinter.Text) 'print the string USB AS
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL) (Copy 1)") 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB
    End Sub
    Private Sub Label30X550QRP()
        pText.Remove(0, pText.Length)
        pText.AppendLine("^XA")   ' Start new document
        pText.AppendLine("^PW1000")   ' Print W=50
        pText.AppendLine("^FWR") ' Rotate 90
        ...
        'pText.AppendLine("^FDQA," & "" & txtCustomer1.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & lbSerialNo1.Text & "|" & txtSetquantity1.Text & "|Auto ^FS") 'New
        pText.AppendLine("^BQ,2,6")  ' QRcode model,magnify
        pText.AppendLine("^FDQA," & "" & lbShiftDate.Text & "|" & txtPartNo1.Text & "P" & "|" & lbRouting.Text & "|" & lbSerialNo1.Text & "|" & txtLotSize1.Text & "^FS") 'Current barcode
        pText.AppendLine("^FO50,1300") ' X,Y position
        'pText.AppendLine("^BQ,2,3")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & txtCustomer1.Text & "|" & txtPartNo1.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & lbSerialNo1.Text & "|" & txtSetquantity1.Text & "|Auto ^FS") 'New
        pText.AppendLine("^BQ,2,4")  ' QRcode model,magnify
        ...
        '------------------------------------------------------------------
        pText.AppendLine("^FO200,1300^A0N,30,35") ' X,Y position
        pText.AppendLine("^FD" & "" & txtPartNo1.Text & "P" & "   Q'ty. " & "" & txtSetquantity1.Text & "^FS") ' part No.
        pText.AppendLine("^FO200,1350^A0N,20,20") ' X,Y position
        pText.AppendLine("^FD" & "" & txtPartDetail1.Text & "^FS") ' Lot No.
        pText.AppendLine("^FO200,1400^A0N,20,20") ' X,Y position
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & " " & "" & txtPartNo1.Text & "P" & " " & "" & lbRouting.Text & " " & "" & lbSerialNo1.Text & "^FS") ' part No.
    ...
    Private Sub ManualPrintLabel()
        ...
        ShowPrintManual()
    End Sub
    Private Sub ManualPrintLabelP()
        Dim k As Integer = cmbSerial.Text
        Dim l As Integer = cmbSerial2.Text
        For count As Integer = k To l
            If count >= k And count <= l Then
                If rbMatrix.Checked = True Then
                    Label30X550MatrixManualP()
                Else
                    Label30X550QRManualP()
                End If
                SaveDataLogManual()
                cnt3 = cmbSerial.Text
                cnt3 = Format(Microsoft.VisualBasic.Right(cnt3.ToString, 4) + 1, "0000")
    ...
    Private Sub Label30X550MatrixManual()
        ...
        '-----------------------------------------------------------
        pText.AppendLine("^FO200,1300^A0N,30,35") ' X,Y position
        pText.AppendLine("^FD" & "" & cmbPartNo.Text & "   Q'ty. " & "" & cmbQuantity.Text & "^FS") ' part No.
        pText.AppendLine("^FO200,1350^A0N,20,20") ' X,Y position
        ...
        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, cmbLotPrinter.Text) 'print the string USB AS
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "ZDesigner ZM400 300 dpi (ZPL) (Copy 1)") 'print the string USB
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB HVCC_Radiator
    End Sub
    Private Sub Label30X550QRManual()
        ...
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB
    End Sub
    Private Sub Label30X550MatrixManualP()
        pText.Remove(0, pText.Length)
        pText.AppendLine("^XA")   ' Start new document
        pText.AppendLine("^PW1000")   ' Print W=50
        pText.AppendLine("^FWR") ' Rotate 90
        ...
        pText.AppendLine("^FO350,300") ' X,Y position
        pText.AppendLine("^A0,65,60")  ' font h,w
        pText.AppendLine("^FD" & "" & cmbPartNo.Text & "P" & "^FS") ' Part No.
        ...
        pText.AppendLine("^FO50,280") ' X,Y position
        pText.AppendLine("^A0,45,40")  ' font h,w
        pText.AppendLine("^FDLot No.: " & "" & lbShiftDate.Text & "|" & cmbPartNo.Text & "P" & "|" & lbRouting.Text & "|" & cmbSerial.Text & "^FS") ' Lot No.
        'Bar Code-------------------------------------------------
        pText.AppendLine("^FO50,80") ' X,Y position
        'pText.AppendLine("^BX,5,200")  ' Matrix Code
        'pText.AppendLine("^FD" & "" & cmbCustomer.Text & "|" & cmbPartNo.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "|Manual ^FS") '
        pText.AppendLine("^BX,8,200")  ' Matrix Code
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & "|" & cmbPartNo.Text & "P" & "|" & lbRouting.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "^FS") 'Current barcode
        pText.AppendLine("^FO50,1300") ' X,Y position
        'pText.AppendLine("^BXB,4,200")  ' Matrix Code Rotate 90
        'pText.AppendLine("^FD" & "" & cmbCustomer.Text & "|" & cmbPartNo.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "|Manual ^FS") '
        pText.AppendLine("^BXB,5,200")  ' Matrix Code Rotate 90
        ...
        '-----------------------------------------------------------
        pText.AppendLine("^FO200,1300^A0N,30,35") ' X,Y position
        pText.AppendLine("^FD" & "" & cmbPartNo.Text & "P" & "   Q'ty. " & "" & cmbQuantity.Text & "^FS") ' part No.
        pText.AppendLine("^FO200,1350^A0N,20,20") ' X,Y position
        pText.AppendLine("^FD" & "" & cmbDetail.Text & "^FS") ' Lot No.
        pText.AppendLine("^FO200,1400^A0N,20,20") ' X,Y position
        pText.AppendLine("^FD" & "" & lbShiftDate.Text & " " & "" & cmbPartNo.Text & "P" & " " & "" & lbRouting.Text & " " & "" & cmbSerial.Text & "^FS") ' part No.
        ...
        'SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL") 'print the string USB HVCC_Radiator
    End Sub
    Private Sub Label30X550QRManualP()
        pText.Remove(0, pText.Length)
        pText.AppendLine("^XA")   ' Start new document
        pText.AppendLine("^PW1000")   ' Print W=50
        pText.AppendLine("^FWR") ' Rotate 90
        ...
        'pText.AppendLine("^FDQA," & "" & cmbCustomer.Text & "|" & cmbPartNo.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "|Manual ^FS") '
        pText.AppendLine("^BQ,2,6")  ' QRcode model,magnify
        pText.AppendLine("^FDQA," & "" & lbShiftDate.Text & "|" & cmbPartNo.Text & "P" & "|" & lbRouting.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "^FS") 'Current barcode
        pText.AppendLine("^FO50,1300") ' X,Y position
        'pText.AppendLine("^BQ,2,3")  ' QRcode model,magnify
        'pText.AppendLine("^FDQA," & "" & cmbCustomer.Text & "|" & cmbPartNo.Text & "|" & lbRouting.Text & "|" & tsToday.Text & "|" & lbShift.Text & "|" & cmbSerial.Text & "|" & cmbQuantity.Text & "|Manual ^FS") '
        pText.AppendLine("^BQ,2,4")  ' QRcode model,magnify
...
#End Region
    ...
    '        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC_Radiator printer
    '    End Sub
    ...
    '        SendToPrinter("ZEBRA ZM400 printing", pText.ToString, "Zebra ZM400 (300 dpi) - ZPL (Copy 1)") 'print the string USB HVCC_Radiator---Zebra ZM400 (300 dpi) - ZPL---
    '    End Sub
...
#Region "[[  Model, Order, Counter  ]]"
    Private Sub txtReadBarCode_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtReadBarCode.KeyPress
        If e.KeyChar = Convert.ToChar(13) Then
            Dim BC As String = txtReadBarCode.Text
            lbPartNo.Text = Format(Microsoft.VisualBasic.Left(BC, 11))
            txtPartNo1.Text = lbPartNo.Text.Trim()
            txtReadBarCode.Clear()
            lbPartNo.Text = "Waiting for next barcode.."
            If txtPartNo1.Text <> "" Then
        ...
        If txtPartNo1.Text <> "" Then
            If Val(txtCounter1.Text) < Val(txtSetquantity1.Text) - 1 Then
                ...
                For Each li In lvPrint.Items
                    If li.SubItems(2).Text = txtPartNo1.Text Then
                        li.SubItems(8).Text() = cnt1
                        li.subitems(9).text() = lbSerialNo1.Text
                        li.subitems(11).text() = lbCounter.Text
...
#End Region
    ...
    Private Sub Timer2_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer2.Tick
        txtReadBarCode.Focus()
    End Sub
    Private Sub txtReadBarCode_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtReadBarCode.GotFocus
        Timer2.Stop()
    End Sub
    Private Sub dgvDatalogger_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvDatalogger.CellMouseClick
        On Error Resume Next
        dgvDatalogger.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.LightBlue
    End Sub
    Private Sub dgvDatalogger_RowLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvDatalogger.RowLeave
        On Error Resume Next
        dgvDatalogger.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.White
    End Sub
    Private Sub dgvDatalogger_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles dgvDatalogger.RowsAdded
        RunItemNo()
    End Sub
    Private Sub cmbPartNo_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbPartNo.TextChanged
        ShowPrintManual()
    End Sub
    Private Sub txtReadBarCode_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtReadBarCode.LostFocus
        Timer2.Start()
    End Sub
    Private Sub rbShow_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbShow.CheckedChanged
        If rbShow.Checked = True Then
            lbP.ForeColor = Color.Red
        Else
            lbP.ForeColor = Color.Blue
        End If
    End Sub
End Class