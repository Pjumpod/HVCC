...
#Region "Transection area"
    Private Sub ShowPrint()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Print,t1.PrintID,t2.PartID,t0.CusID")
    ...
    Private Sub UpdatePrint()
        ...
        sb.Append("[LeakTest]='" & txtLeakTest.Text & "',")
    ...
    Private Sub SavePrint()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("INSERT INTO tbPrintLabel([PartNo],[CustomerNo],[LeakTest],[Counter],[Serial],[EachCount],[Print])")
        sb.Append(" Values (" & Val(cmbPartID.Text) & "," & Val(cmbCusID.Text) & ",'" & txtLeakTest.Text & "','0','0000','0000','" & cmbPrintUse.Text & "')")
    ...
    Private Sub PrintView()
        ...
        With dgvAddPrinting
            If .RowCount > 0 Then
                ...
                .Columns(3).HeaderText = "Part No"
                .Columns(4).HeaderText = "Part No. Detail"
                .Columns(5).HeaderText = "Leak Test"
                .Columns(6).HeaderText = "Quantity"
                .Columns(7).HeaderText = "Lot Size"
                .Columns(8).HeaderText = "Print"
                .Columns(0).Width = 60
                '.Columns(1).Width = 60
                '.Columns(2).Width = 60
                .Columns(1).Width = 150
...
#End Region
    ...
    Private Sub dgvAddPrinting_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvAddPrinting.CellMouseClick
        ...
        cmbCusNo1.Text = dgvAddPrinting.Rows(x).Cells(1).Value()
        cmbCusNo2.Text = dgvAddPrinting.Rows(x).Cells(2).Value()
        cmbPartNo.Text = dgvAddPrinting.Rows(x).Cells(3).Value()
        txtLeakTest.Text = dgvAddPrinting.Rows(x).Cells(5).Value()
        cmbPrintUse.Text = dgvAddPrinting.Rows(x).Cells(8).Value()
        lbID.Text = dgvAddPrinting.Rows(x).Cells(9).Value()
        cmbPartID.Text = dgvAddPrinting.Rows(x).Cells(10).Value()
        cmbCusID.Text = dgvAddPrinting.Rows(x).Cells(11).Value()
    End Sub
