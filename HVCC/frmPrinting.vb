...
#Region "Transection area"
    Private Sub ShowPrint()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("SELECT t1.Model,t1.Order,t0.CustomerPartNo,t0.CustomerPartNo2,t2.PartNo,t2.PartNoDetail,t1.LeakTest,t2.Quantity,t2.LotSize,t1.Print,t1.PrintID,t2.PartID,t0.CusID")
    ...
    Private Sub UpdatePrint()
        ...
        sb.Append("[Model]='" & cmbModel.Text & "',[Order]='" & cmbOrder.Text & "',[LeakTest]='" & txtLeakTest.Text & "',")
    ...
    Private Sub SavePrint()
        ...
        Dim sb As New StringBuilder()
        sb.Remove(0, sb.Length)
        sb.Append("INSERT INTO tbPrintLabel([PartNo],[CustomerNo],[Model],[Order],[LeakTest],[Counter],[Serial],[EachCount],[Print])")
        sb.Append(" Values (" & Val(cmbPartID.Text) & "," & Val(cmbCusID.Text) & ",'" & cmbModel.Text & "','" & cmbOrder.Text & "','" & txtLeakTest.Text & "','0','0000','0000','" & cmbPrintUse.Text & "')")
    ...
    Private Sub PrintView()
        ...
        With dgvAddPrinting
            If .RowCount > 0 Then
                ...
                .Columns(3).HeaderText = "Customer Part No.1"
                .Columns(4).HeaderText = "Customer Part No.2"
                .Columns(5).HeaderText = "Part No"
                .Columns(6).HeaderText = "Part No. Detail"
                .Columns(7).HeaderText = "Leak Test"
                .Columns(8).HeaderText = "Quantity"
                .Columns(9).HeaderText = "Lot Size"
                .Columns(10).HeaderText = "Print"
                .Columns(0).Width = 60
                .Columns(1).Width = 60
                .Columns(2).Width = 60
                .Columns(3).Width = 150
...
#End Region
    ...
    Private Sub dgvAddPrinting_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvAddPrinting.CellMouseClick
        ...
        cmbCusNo1.Text = dgvAddPrinting.Rows(x).Cells(3).Value()
        cmbCusNo2.Text = dgvAddPrinting.Rows(x).Cells(4).Value()
        cmbPartNo.Text = dgvAddPrinting.Rows(x).Cells(5).Value()
        txtLeakTest.Text = dgvAddPrinting.Rows(x).Cells(7).Value()
        cmbPrintUse.Text = dgvAddPrinting.Rows(x).Cells(10).Value()
        lbID.Text = dgvAddPrinting.Rows(x).Cells(11).Value()
        cmbPartID.Text = dgvAddPrinting.Rows(x).Cells(12).Value()
        cmbCusID.Text = dgvAddPrinting.Rows(x).Cells(13).Value()
    End Sub
